...
class TestTextLearningNeuralIntegration:
    ...
    def test_text_embeddings_compatible_with_neurons(self):
        ...
        neuron = Neuron(input_size=64, learning_rate=0.01)
        ...
        # Activate neuron with embedding
        output = neuron.activate(embedding)
    ...
    def test_text_embeddings_batch_processing(self):
        ...
        layer = NeuralLayer(input_size=32, layer_size=5, learning_rate=0.01)
        ...
        # Activate layer with embeddings
        results = []
        for emb in embeddings:
            output = layer.activate(emb)
    ...
    def test_text_learning_updates_affect_neural_processing(self):
        """Test that learning updates grow vocabulary and neurons can process new embeddings"""
        text_module = TextLearningModule(embedding_dim=16)
        neuron = Neuron(input_size=16, learning_rate=0.01)

        # Initial learning
        text_module.learn_from_text("test")
        initial_vocab = text_module.get_stats()['vocabulary_size']

        # More learning with new words
        text_module.learn_from_text("word example context hello")
        final_vocab = text_module.get_stats()['vocabulary_size']

        # Vocabulary should have grown
        assert final_vocab > initial_vocab

        # All new words should produce valid neuron activations
        for word in ['test', 'word', 'example']:
            idx = text_module.token_to_idx[word]
            emb = text_module.embeddings[idx]
            output = neuron.activate(emb)
            assert np.isfinite(output)
...
class TestMemorySystemsIntegration:
    ...
    def test_episodic_text_memory_integration(self):
        ...
        text_module = TextLearningModule(embedding_dim=64)
        memory = EpisodicMemory(state_size=64, max_episodes=100)

        # Learn and store episode
        result = text_module.learn_from_text("important event happened")

        # Create state vector from the learned vocabulary
        state = np.random.randn(64).astype(np.float32)
        sensory_data = {'text': np.random.randn(64).astype(np.float32)}
        context = {
            'type': 'text_learning',
        ...
        episode = memory.store(state, sensory_data, context=context)

        # Retrieve using the state as cue
        retrieved = memory.retrieve(cue=state, n_episodes=1)
    ...
    def test_semantic_memory_text_concepts(self):
        ...
        for concept in concepts:
            ...
            semantic.add_concept(concept, embedding)

        # Query related concepts
        dog_idx = text_module.token_to_idx['dog']
        dog_emb = text_module.embeddings[dog_idx]
        related = semantic.query(cue_embedding=dog_emb, n_results=3)
...
class TestLearningEngineIntegration:
    ...
    def test_learning_engine_with_text(self):
        ...
        text_module = TextLearningModule(embedding_dim=32)
        engine = LearningEngine(state_dim=32, learning_rate=0.01)

        # Learn text
        text_module.learn_from_text("learning test")

        # Get embeddings for learning - verify they have expected shape
    ...
    def test_curriculum_with_text_learning(self):
        ...
        # CurriculumLevel is an enum with predefined levels
        levels = [
            CurriculumLevel.LEVEL_1_BASIC,
            CurriculumLevel.LEVEL_2_ASSOCIATION,
        ]
        ...
        # Determine level based on vocabulary
...
class TestEndToEndIntegration:
    ...
    def test_multi_module_coordination(self):
        ...
        for word in ['coordinate', 'test']:
            ...
            # Layer 1
            out1 = layer1.activate(emb)

            # Layer 2
            if isinstance(out1, np.ndarray):
                out2 = layer2.activate(out1)
...
class TestErrorHandlingIntegration:
    ...
    def test_invalid_input_handling(self):
        ...
        # Empty input - TextLearningModule may still process special tokens
    ...
    def test_dimension_mismatch_handling(self):
        ...
        text_module = TextLearningModule(embedding_dim=64)
        neuron = Neuron(input_size=32)  # Different dimension

        text_module.learn_from_text("test")
        emb = text_module.embeddings[text_module.token_to_idx['test']]

        # Truncate to match neuron input size and verify it processes
        result = neuron.activate(emb[:32])

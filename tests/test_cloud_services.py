...
def _mock_save_state(path):
    """Mock save_state that actually creates the file on disk."""
    with open(path, 'wb') as f:
        f.write(b'mock_checkpoint_data')
    return True


def _setup_service_mocks(service):
    """Set attributes that _initialize_system would normally create."""
    service.system = MagicMock()
    service.system.frame_count = 0
    service.system.save_state.side_effect = _mock_save_state
    service.text_module = None
    service.text_api = None
...
class TestMetricsCollection:
    ...
    def test_metrics_initialization(self):
        ...
        import cloud.salad_service as cs
        orig_prometheus = cs.HAS_PROMETHEUS
        orig_counter = getattr(cs, 'Counter', None)
        orig_gauge = getattr(cs, 'Gauge', None)
        orig_histogram = getattr(cs, 'Histogram', None)

        try:
            cs.HAS_PROMETHEUS = True
            cs.Counter = mock_counter
            cs.Gauge = mock_gauge
            cs.Histogram = mock_histogram
            ...
            cs.HAS_PROMETHEUS = orig_prometheus
            if orig_counter is not None:
                cs.Counter = orig_counter
            elif hasattr(cs, 'Counter'):
                delattr(cs, 'Counter')
            if orig_gauge is not None:
                cs.Gauge = orig_gauge
            elif hasattr(cs, 'Gauge'):
                delattr(cs, 'Gauge')
            if orig_histogram is not None:
                cs.Histogram = orig_histogram
            elif hasattr(cs, 'Histogram'):
                delattr(cs, 'Histogram')
        ...
        with patch.object(AtlasSaladService, '_initialize_system'):
            with tempfile.TemporaryDirectory() as tmpdir:
                with patch.dict(os.environ, {'ATLAS_CHECKPOINT_DIR': tmpdir}):
                    service = AtlasSaladService()
                    _setup_service_mocks(service)
                    # Override to simulate save failure (no file created)
                    service.system.save_state.side_effect = None

...
def create_processors(config: SystemConfig) -> tuple:
    """
...
def create_system(config: SystemConfig, visual_processor: VisualProcessor, audio_processor: AudioProcessor) -> SelfOrganizingAVSystem:
    """
...
def load_checkpoint(system: SelfOrganizingAVSystem, config: SystemConfig, session_name: str = None) -> bool:
    """
...
def save_checkpoint(system: SelfOrganizingAVSystem, config: SystemConfig, session_name: str = None, is_final: bool = False) -> bool:
    """
...
def run_live_session(system: SelfOrganizingAVSystem, config: SystemConfig) -> None:
    """
    ...
    capture = AVCapture(
        ...
        sample_rate=config.get_audio_config()["sample_rate"],
...
def run_file_session(system: SelfOrganizingAVSystem, video_file: str, config: SystemConfig) -> None:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Self-Organizing Audio-Visual Learning System')
    parser.add_argument('--config', type=str, default=None, help='Path to configuration file')
    parser.add_argument('--log-level', type=str, default='INFO', help='Logging level')
    parser.add_argument('--video-file', type=str, default=None, help='Path to video file (for file-based session)')
    ...
    parser.add_argument('--checkpoint-name', type=str, help='Specific checkpoint to load')
    ...
    if args.no_checkpoints:
        config.update("checkpointing", "enabled", False)
    if args.no_load_checkpoint:
        config.update("checkpointing", "load_latest", False)
    ...
    if args.checkpoint_name and not args.no_load_checkpoint:
        checkpoint_dir = config.get_checkpointing_config()["checkpoint_dir"]

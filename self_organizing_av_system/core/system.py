...
class SelfOrganizingAVSystem:
    ...
    def __init__(
        ...
        self._auto_checkpoint_enabled = config.get("auto_checkpoint", False)
        self._checkpoint_interval = config.get("checkpoint_interval_minutes", 10)
        if CHECKPOINT_MANAGER_AVAILABLE and self._auto_checkpoint_enabled:
            self._init_checkpoint_manager()
    
    def _init_checkpoint_manager(self):
        """Initialize the checkpoint manager for automatic checkpointing."""
        checkpoint_config = {
            "checkpoint_dir": self.config.get("checkpoint_dir", "checkpoints"),
            "auto_checkpoint": self._auto_checkpoint_enabled,
            "checkpoint_interval_minutes": self._checkpoint_interval,
            "max_checkpoints": self.config.get("max_checkpoints", 10),
            "compression": self.config.get("checkpoint_compression", True),
            "cloud_sync": self.config.get("cloud_sync", False),
        ...
        self._checkpoint_manager = get_checkpoint_manager(checkpoint_config)
        
        # Start auto-checkpoint if enabled
        if self._auto_checkpoint_enabled:
            self._checkpoint_manager.start_auto_checkpoint(self, self._checkpoint_interval)
            logger.info(f"Auto-checkpoint enabled (interval: {self._checkpoint_interval} min)")
    ...
    def save_checkpoint_with_manager(
        ...
        Returns:
            Checkpoint info as dictionary
        """
        if not CHECKPOINT_MANAGER_AVAILABLE:
            # Fallback to legacy save_checkpoint
        ...
        info = self._checkpoint_manager.save_checkpoint(
            system=self,
            name=name,
            metadata=metadata,
            compress=compress,
    ...
    def load_checkpoint_with_manager(
        self,
        ...
        system, info = self._checkpoint_manager.load_checkpoint(
            version=version,
            name=name
        )
        return info.to_dict()
    
    def rollback_checkpoint(self, steps: int = 1) -> Dict[str, Any]:
        """
        ...
        info = self._checkpoint_manager.rollback(steps=steps)
        return info.to_dict()
    
    def list_checkpoints_with_manager(self, include_metadata: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        return self._checkpoint_manager.list_checkpoints(include_metadata=include_metadata)
    
    def get_checkpoint_stats(self) -> Dict[str, Any]:
        """Get checkpoint manager statistics."""
        if not CHECKPOINT_MANAGER_AVAILABLE or self._checkpoint_manager is None:
            return {"available": False}
        
        return self._checkpoint_manager.get_stats()
    
    def sync_checkpoints_to_cloud(self, version: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        synced = self._checkpoint_manager.sync_to_cloud(version=version)
        return [info.to_dict() for info in synced]
    
    def sync_checkpoints_from_cloud(self) -> List[Dict[str, Any]]:
        """
        ...
        downloaded = self._checkpoint_manager.sync_from_cloud()
        return [info.to_dict() for info in downloaded]
